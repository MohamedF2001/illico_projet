...
class AdminLivraisonsPage extends ConsumerWidget {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          SizedBox(
            width: 120, // Largeur fixe pour éviter l'erreur d'infinite width
            child: Padding(
              padding: const EdgeInsets.only(right: 8),
              child: ElevatedButton.icon(
                onPressed: () => _showAddLivraisonForm(context, ref),
  ...
  // ✅ FIX 2 : Fonction sécurisée pour formater les dates sans erreur de locale
  String _safeFormatDate(DateTime date) {
    try {
      return Formatters.date(date);
    } catch (_) {
      // Fallback simple si intl n'est pas initialisé
...
class _AddLivraisonDialogState extends State<_AddLivraisonDialog> {
  ...
  @override
  void dispose() {
    // ✅ Bonne pratique : libérer les contrôleurs
    _clientCtrl.dispose();
    _departCtrl.dispose();
